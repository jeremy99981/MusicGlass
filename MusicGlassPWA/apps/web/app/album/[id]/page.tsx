...
export default function AlbumPage() {
  ...
  const playableTracks = useMemo(() => (data?.tracks ?? [])
    ...
    .map((item) => normalizeTrack({
      id: item.id,
      title: item.title,
      artist: item.artist || data?.artist || data?.title,
      album: data?.title,
  ...
  if (error || !data) return <DetailError message="Impossible de charger l’album." onBack={() => router.back()} />;
  ...
  return (
    <main className={styles.detailPage}>
      ...
      <DetailHeader
        ...
        onPlay={playAll}
      />
      <section className={styles.trackList} aria-label="Titres de l’album">
...
function DetailHeader({ title, subtitle, artwork, trackCount, onPlay }: {
  title: string;
  subtitle?: string;
  artwork?: string;
  trackCount: number;
  ...
  return (
    <header className={styles.detailHeader}>
      ...
      <div className={styles.detailCopy}>
        <span className={styles.kicker}>Album</span>
        <h1>{title}</h1>
